...
class CornersProblem(search.SearchProblem):
    ...
    def goalTest(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return len(state[1]) == 0  # Check if all corners have been visited
    ...
    def getResult(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        if not self.walls[nextx][nexty]:
            nextState = ((nextx, nexty), tuple(c for c in state[1] if c != (nextx, nexty)))
            return nextState
        else:
            return state
...
def cornersHeuristic(state, problem):
    ...
    # If all corners have been visited, we're done
    ...
    # Function to calculate the minimum spanning tree of a set of points
    # using Prim's algorithm
    def minimumSpanningTree(points):
        if len(points) <= 1:
            return 0
        remaining = list(points).copy()
        tree = [remaining.pop()]
        mst = 0
        while remaining:
            shortest_distance = min(manhattanDistance(p1, p2) for p1 in tree for p2 in remaining)
            mst += shortest_distance
            for p1 in tree:
                for p2 in remaining:
                    if manhattanDistance(p1, p2) == shortest_distance:
                        tree.append(p2)
                        remaining.remove(p2)
                        break
        return mst

    # The heuristic is the distance to the nearest corner plus the minimum
    # spanning tree of the remaining corners
    distances = [manhattanDistance(currentPosition, corner) for corner in remainingCorners]
    return min(distances) + minimumSpanningTree(remainingCorners)
